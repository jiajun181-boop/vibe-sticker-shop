...
export default async function QuotePrintPage({ searchParams }) {
  ...
  if (!data) {
    return (
      <main className="mx-auto max-w-3xl p-10">
        <h1 className="text-2xl font-semibold text-gray-900">Quote data missing</h1>
        <p className="mt-2 text-sm text-gray-500">Please generate the quote from a product page.</p>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto max-w-4xl bg-white p-8 text-gray-900">
      <QuotePrintClient />

      <header className="border-b border-gray-200 pb-5">
        <div className="flex items-start justify-between gap-6">
          <div>
            <p className="text-[11px] uppercase tracking-[0.2em] text-gray-500">La Lunar Printing Inc.</p>
            <h1 className="mt-2 text-3xl font-bold">Quote</h1>
            <p className="mt-1 text-sm text-gray-500">Generated {new Date(data.generatedAt || Date.now()).toLocaleString("en-CA")}</p>
          </div>
          <div className="text-right">
            <p className="text-[11px] uppercase tracking-[0.2em] text-gray-500">Reference</p>
      ...
      <section className="mt-6 grid gap-6 md:grid-cols-2">
        <div className="rounded-xl border border-gray-200 p-4">
          <p className="text-xs font-semibold uppercase tracking-[0.15em] text-gray-500">Product</p>
          <p className="mt-2 text-lg font-semibold">{data.productName || "-"}</p>
          <p className="mt-1 text-xs text-gray-500">Category: {data.category || "-"}</p>
          <p className="mt-1 text-xs text-gray-500">Qty: {data.quantity || "-"}</p>
        </div>

        <div className="rounded-xl border border-gray-200 p-4">
          <p className="text-xs font-semibold uppercase tracking-[0.15em] text-gray-500">SLA Window</p>
      ...
      <section className="mt-6 rounded-xl border border-gray-200 p-4">
        <p className="text-xs font-semibold uppercase tracking-[0.15em] text-gray-500">Configuration</p>
        <div className="mt-3 grid gap-2 md:grid-cols-2">
          {selections.map((row, idx) => (
            <div key={`${row.label || "row"}-${idx}`} className="flex items-center justify-between rounded-lg border border-gray-100 px-3 py-2 text-sm">
      ...
      <section className="mt-6 rounded-xl border border-gray-200 p-4">
        <p className="text-xs font-semibold uppercase tracking-[0.15em] text-gray-500">Pricing (CAD)</p>
