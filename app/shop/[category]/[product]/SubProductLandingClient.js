    ...
    >
      ...
      <div className="p-3 sm:p-4">
        ...
        {selectedSpec === product.slug && (
          <p className="mt-1 text-[10px] font-semibold uppercase tracking-[0.15em] text-gray-600">Selected spec</p>
        )}
        {product.description && (
        ...
        {price > 0 && (
          <p className="mt-2 text-sm font-bold text-gray-900">{t("product.from", { price: formatCad(price) })}</p>
        )}
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-3 sm:p-4">
        <div className="flex items-start gap-2">
          ...
          {selectedSpec === product.slug && (
            <span className="text-[10px] font-semibold uppercase tracking-[0.15em] text-gray-600">Selected</span>
          )}
        </div>
        <h3 className="mt-1 text-sm font-semibold text-gray-900 leading-snug">{product.name}</h3>
        {product.description && (
          <p className="mt-1 text-[11px] text-gray-500 line-clamp-2 sm:line-clamp-3">{product.description}</p>
        )}
        <div className="mt-auto pt-2 flex items-center gap-3 flex-wrap">
          {sizeCount > 0 && (
            <span className="text-[11px] text-gray-400">{sizeCount} {t("mp.landing.options")}</span>
          )}
          {price > 0 && (
            <span className="text-sm font-bold text-gray-900">{t("product.from", { price: formatCad(price) })}</span>
          )}
          <div className="ml-auto flex items-center gap-2">
            <QuickAddButton product={product} />
            <span className="rounded-full bg-gray-900 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.15em] text-white transition-colors group-hover:bg-black">
              {t("mp.landing.viewOrder")}
            </span>
          </div>
        </div>
          ...
          <div className="mt-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1 rounded-lg border border-gray-200 bg-white p-0.5">
              ...
              <button
                ...
                className={`rounded-md p-1.5 transition-colors ${
                  viewMode === "list" ? "bg-gray-900 text-white" : "text-gray-400 hover:text-gray-700"
                }`}
            ...
            <div className="mt-4 grid gap-3 grid-cols-1 sm:grid-cols-2">
              {crossSellGroups.map((sg) => (
                <Link
                  key={sg.slug}
                  href={sg.href}
                  className="group flex items-center gap-4 rounded-2xl border border-gray-200 bg-white p-4 transition-all hover:-translate-y-0.5 hover:shadow-md hover:border-gray-400"
                >
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gray-100 text-gray-400 group-hover:bg-gray-200 transition-colors">
                  ...
                  <svg className="h-4 w-4 shrink-0 text-gray-400 group-hover:text-gray-600 transition-transform group-hover:translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
            ...
            <div className="mt-3 flex gap-2 overflow-x-auto scrollbar-hide scroll-fade pb-2">
              {siblingSubGroups.map((sg) => (
                <Link
                  key={sg.slug}
                  href={sg.href}
                  className="group shrink-0 flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2.5 transition-all hover:-translate-y-0.5 hover:shadow-md hover:border-gray-400"
                >
                  <span className="text-sm font-medium text-gray-700 group-hover:text-gray-900 whitespace-nowrap">
                    {sg.title}
                  </span>
                  <svg className="h-3 w-3 text-gray-400 group-hover:text-gray-600 transition-transform group-hover:translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
                  </svg>
                </Link>
              ))}
