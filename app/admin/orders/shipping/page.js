...
const STATUS_OPTIONS = ["all", ...Object.keys(SHIPMENT_STATUS)];

const CARRIER_OPTIONS = ["all", ...Object.keys(SHIPPING_CARRIERS)];

const TABS = [
  { key: "shipments", label: "Fulfillment Records" },
  { key: "batch", label: "Batch Dispatch" },
...
function ShippingContent() {
  ...
  function switchTab(tab) {
    ...
    router.push(`/admin/orders/shipping${params.toString() ? "?" + params : ""}`);
  }
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <Link
            ...
            className="text-[11px] text-[#666] underline hover:text-black hover:no-underline"
          >
            Orders Center / Fulfillment
          </Link>
          <h1 className="mt-1 text-xl font-bold text-black">Order Fulfillment</h1>
          <p className="mt-1 text-xs text-[#999]">
            Create shipment records, track dispatch, and move orders through fulfillment from one order-centered workspace.
          </p>
        </div>
      </div>
...
function ShipmentsTab() {
  ...
  function updateParams(updates) {
    ...
    router.push(`/admin/orders/shipping?${params}`);
  }
  ...
  async function handleBulkAction(newStatus) {
    ...
    const label = getShipmentStatusLabel(newStatus);
  ...
  return (
    <>
      <div className="flex flex-wrap gap-2">
        ...
        <Link
          href={buildOrderCenterHref("shipped")}
          className="rounded-[3px] border border-teal-200 bg-teal-50 px-3 py-1.5 text-xs font-medium text-teal-700 hover:bg-teal-100"
        >
          Shipped orders
        </Link>
      </div>
      ...
      <div className="flex flex-col gap-3 rounded-[3px] border border-[#e0e0e0] bg-white p-4 shadow-sm sm:flex-row sm:flex-wrap sm:items-center">
          ...
          >
            {STATUS_OPTIONS.map((status) => (
              <option key={status} value={status}>
                {status === "all" ? "All Statuses" : getShipmentStatusLabel(status)}
              </option>
            ))}
          </select>
        </div>
          ...
          >
            {CARRIER_OPTIONS.map((carrier) => (
              <option key={carrier} value={carrier}>
                {carrier === "all" ? "All Carriers" : getCarrierLabel(carrier)}
              </option>
            ))}
          </select>
        </div>
        ...
        <form onSubmit={handleSearch} className="flex gap-2 sm:ml-auto">
          <input
            ...
            placeholder="Order ID, customer email, or tracking #"
      ...
      <div className="overflow-hidden rounded-[3px] border border-[#e0e0e0] bg-white shadow-sm">
        {loading ? (
          ...
          <div className="flex h-48 flex-col items-center justify-center gap-2 text-sm text-[#999]">
            <p>No fulfillment records found</p>
            <p className="text-xs">
              Create a shipment record or return to the order views
            </p>
          </div>
        ) : (
          <>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          ...
                          <div className="mt-1 space-y-0.5">
                            <p className="truncate text-[11px] text-[#666]">
                              {s.orderSummary?.customerName || s.orderSummary?.customerEmail || "â€”"}
                            </p>
                            <p className="text-[10px] uppercase tracking-wide text-[#999]">
                              {formatProductionState(s.orderSummary?.productionStatus)}
                            </p>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`inline-block rounded-[2px] px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${
                              getShipmentStatusColor(s.status)
                            }`}
                          >
  ...
  async function handleSubmit(e) {
    ...
    try {
      const payload = {
        ...
        carrier: normalizeCarrierCode(form.carrier),
    ...
    >
      <div className="mx-4 w-full max-w-lg rounded-[3px] bg-white p-6 shadow-xl">
        <h3 className="mb-4 text-sm font-bold text-[#111]">
          {isEdit ? "Edit fulfillment record" : "Create fulfillment record"}
        </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            ...
            <select
              ...
              className="w-full rounded-[3px] border border-[#d0d0d0] px-3 py-2 text-sm outline-none focus:border-black"
            >
              {CARRIER_OPTIONS.filter((carrier) => carrier !== "all").map((carrier) => (
                <option key={carrier} value={carrier}>
...
function BatchShipTab() {
  ...
  async function loadPendingOrders() {
    ...
    try {
      const res = await fetch(
        "/api/admin/orders?production=ready_to_ship&limit=100&sort=createdAt&order=desc"
      );
      const data = await res.json();
  ...
  async function handleBatchShip() {
    ...
    if (mode === "paste") {
      ...
      if (selectedOrderIds.length === 0) {
        setResults({ error: "Select at least one ready-to-ship order." });
        return;
      }
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="overflow-hidden rounded-[3px] border border-[#e0e0e0] bg-white shadow-sm">
          {loadingOrders ? (
            ...
            <div className="flex h-32 flex-col items-center justify-center gap-1 text-sm text-[#999]">
              <p>No ready-to-ship orders found</p>
              <p className="text-xs">
                Orders must be in the ready-to-ship queue to appear here
              </p>
            </div>
          ) : (
            <div className="divide-y divide-[#e0e0e0]">
              <div className="bg-[#fafafa] px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-[#999]">
                {pendingOrders.length} ready-to-ship orders available —{" "}
      ...
      >
        {shipping ? "Creating shipments..." : "Ship selected orders"}
      </button>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div className="rounded-[3px] border border-[#e0e0e0] bg-white p-4 shadow-sm">
          <p className="text-xs font-medium text-[#999]">Dispatches This Week</p>
          <p className="mt-1 text-2xl font-semibold text-black">
            {stats.shipmentsThisWeek}
          </p>
        </div>
        <div className="rounded-[3px] border border-[#e0e0e0] bg-white p-4 shadow-sm">
          <p className="text-xs font-medium text-[#999]">Dispatches This Month</p>
          <p className="mt-1 text-2xl font-semibold text-black">
            {stats.shipmentsThisMonth}
          </p>
        </div>
        <div className="rounded-[3px] border border-[#e0e0e0] bg-white p-4 shadow-sm">
          <p className="text-xs font-medium text-[#999]">Average Delivery Time</p>
          <p className="mt-1 text-2xl font-semibold text-blue-600">
            {formatDeliveryTime(stats.avgDeliveryHours)}
          </p>
        </div>
        <div className="rounded-[3px] border border-[#e0e0e0] bg-white p-4 shadow-sm">
          <p className="text-xs font-medium text-[#999]">Delivered Orders</p>
      ...
      <div className="rounded-[3px] border border-[#e0e0e0] bg-white p-4 shadow-sm">
        ...
        {Object.keys(byStatus).length === 0 ? (
          <p className="text-xs text-[#999]">No fulfillment data yet</p>
        ) : (
                  ...
                  <div className="flex-1">
                    <div
                      className={`h-5 rounded-[2px] ${
                        getShipmentStatusColor(status)?.split(" ")[0] || "bg-gray-200"
                      }`}
      ...
      <div className="rounded-[3px] border border-[#e0e0e0] bg-white p-4 shadow-sm">
        ...
        {Object.keys(byCarrier).length === 0 ? (
          ...
          <div className="space-y-2">
            {CARRIER_OPTIONS.filter((carrier) => carrier !== "all").map((carrier) => {
              const count = byCarrier[carrier] || 0;
              if (count === 0) return null;
              ...
              return (
                <div key={carrier} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <div
                      className={`h-5 rounded-[2px] ${
                        getCarrierColor(carrier)?.split(" ")[0] || "bg-gray-200"
                      }`}
                      style={{ width: `${Math.max(pct, 2)}%` }}
                    />
                  </div>
