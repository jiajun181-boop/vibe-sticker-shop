...
export default function AFrameSignOrderClient({ productImages = [] }) {
  ...
  const buildCartItem = useCallback(() => {
    ...
    return {
      ...
      options: {
        ...
        height: size.h,
        material,
        materialLabel: t(`af.material.${material}`),
        sides,
        frame,
        frameLabel: t(`af.frame.${frame}`),
  ...
  }, [quoteData, activeQty, adjustedSubtotal, size, material, sides, frame, uploadedFile, t]);

  const { handleAddToCart, handleBuyNow, buyNowLoading } = useConfiguratorCart({
    buildCartItem,
    successMessage: t("af.addedToCart"),
  });
        ...
        <div className="mx-auto flex max-w-lg items-center gap-3">
          <div className="min-w-0 flex-1">
            {quoteLoading ? (
              ...
              <p className="text-sm text-gray-400">{disabledReason || t("af.selectOptions")}</p>
            )}
          </div>
          <button
            type="button"
