...
export default function AluminumSignOrderClient({ productImages = [] }) {
  ...
  const buildCartItem = useCallback(() => {
    ...
    return {
      ...
      options: {
        ...
        height: size.h,
        thickness,
        materialLabel: t(`al.thick.${thickness}`),
        sides,
        corners,
        holes,
        reflective,
  ...
  }, [quoteData, activeQty, adjustedSubtotal, size, thickness, sides, corners, holes, reflective, uploadedFile, t]);

  const { handleAddToCart, handleBuyNow, buyNowLoading } = useConfiguratorCart({
    buildCartItem,
    successMessage: t("al.addedToCart"),
  });
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      {(() => {
        const faqItems = getConfiguratorFaqs("aluminum-signs");
