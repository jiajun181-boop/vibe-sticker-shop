...
export default function XBannerStandOrderClient({ productImages = [] }) {
  ...
  }, [quoteData, activeQty, totalCents, size, orderType, material, stand, uploadedFile, t]);

  const { handleAddToCart, handleBuyNow, buyNowLoading } = useConfiguratorCart({
    buildCartItem,
    successMessage: t("xb.addedToCart"),
  });
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      {(() => {
        const faqItems = getConfiguratorFaqs("x-banner-stands");
        ...
        <div className="mx-auto flex max-w-lg items-center gap-3">
          <div className="min-w-0 flex-1">
            {quoteLoading ? (
              ...
              <p className="text-sm text-gray-400">{disabledReason || t("xb.selectOptions")}</p>
            )}
          </div>
          <button type="button" onClick={() => handleAddToCart({ artworkIntent })} disabled={!canAddToCart}
