...
export default function VinylBannerOrderClient({ productImages = [] }) {
  ...
  const buildCartItem = useCallback(() => {
    ...
    return {
      ...
      options: {
        ...
        height: size.h,
        material: materialId,
        materialLabel: t(`vb.material.${materialId}`),
        finishing: finishingId,
        finishingLabel: t(`vb.finishing.${finishingId}`),
  ...
  }, [quoteData, activeQty, totalCents, size, materialId, finishingId, grommetSpacing, polePocketPos, polePocketSize, sidesId, turnaroundId, uploadedFile, t]);

  const { handleAddToCart, handleBuyNow, buyNowLoading } = useConfiguratorCart({
    buildCartItem,
    successMessage: t("vb.addedToCart"),
  });
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      {productImages.length > 0 && (
        <div className="mb-8">
          <ImageGallery images={productImages} productName={t("vb.title")} />
        </div>
      )}
      ...
      {/* FAQ */}
      {(() => {
        const faqItems = getConfiguratorFaqs("vinyl-banners");
