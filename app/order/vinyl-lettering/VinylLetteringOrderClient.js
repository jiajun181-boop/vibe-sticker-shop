...
export default function VinylLetteringOrderClient({ productImages = [] }) {
  ...
  const buildCartItem = useCallback(() => {
    ...
    return {
      ...
      options: {
        type,
        ...(isLogo
          ? { widthIn: parsedLogoW, heightIn: parsedLogoH, logoMode, sizeLabel: `${parsedLogoW}" × ${parsedLogoH}"` }
          : { letterHeight: heightId, letterHeightLabel: letterHeight.label, letteringText: letteringText.trim() || null, font: fontId, sizeLabel: letterHeight.label }),
        ...(!(isLogo && logoMode === "printed") && { vinylColor: color, color }),
        material,
  ...
  const { handleAddToCart, handleBuyNow, buyNowLoading } = useConfiguratorCart({
    buildCartItem,
    successMessage: t("vl.addedToCart"),
  });
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      {(() => {
        const faqItems = getConfiguratorFaqs("vinyl-lettering");
