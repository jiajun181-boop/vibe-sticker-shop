...
export default function DieCutStickerOrderClient() {
  ...
  const fetchQuote = useCallback(() => {
    ...
    fetch("/api/quote", {
      ...
      body: JSON.stringify({ slug: "die-cut-stickers", quantity: activeQty, widthIn: size.w, heightIn: size.h, sides: "single" }),
      signal: ac.signal,
    })
        ...
        <aside className="hidden lg:col-span-2 lg:block">
          <div className="sticky top-24 space-y-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="flex items-center justify-center gap-4 pt-2 text-[11px] text-gray-400">
              <span>{t("dc.badge.waterproof")}</span>
              <span className="text-gray-300">|</span>
              <span>{t("dc.badge.shipping")}</span>
            </div>
          </div>
        </aside>
      </div>
