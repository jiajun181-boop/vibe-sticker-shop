  ...
  return (
    <>
      ...
      <div className={`fixed inset-y-0 left-0 z-[61] w-80 border-r border-[var(--color-gray-200)] bg-white shadow-lg transition-transform duration-200 ease-out ${open ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <nav className="flex-1 overflow-y-auto" style={{ height: "calc(100% - 57px - 72px)" }}>
          <div className="px-4">
              ...
              return (
                <div key={key} className="border-b border-[var(--color-gray-200)]">
                  ...
                  >
                    ...
                    <ChevronIcon
                      className={`h-5 w-5 shrink-0 text-[var(--color-gray-400)] transition-transform duration-200 ease-in-out ${isOpen ? "rotate-180" : ""}`}
                    />
                  </button>

                  <div
                    className="overflow-hidden transition-[max-height] duration-200 ease-in-out"
                    style={{ maxHeight: isOpen ? `${subGroups.length * 40 + 8}px` : "0px" }}
                  >
                    <div className="pb-2">
                      {subGroups.map((sg) => (
                        <Link
        ...
        {/* Bottom buttons */}
        <div className="absolute bottom-0 left-0 right-0 border-t border-[var(--color-gray-200)] bg-white px-4 py-3 pb-safe flex gap-3">
          <Link
            href="/quote"
            onClick={onClose}
            className="btn-primary-pill flex-1 text-center px-3 py-2.5 text-sm font-semibold"
          >
            {t("nav.getQuote")}
          </Link>
          <Link
            href="/shop"
            onClick={onClose}
            className="btn-secondary-pill flex-1 text-center px-3 py-2.5 text-sm font-semibold"
          >
            {t("nav.shopAll")}
          </Link>
        </div>
