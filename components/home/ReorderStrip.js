    ...
    <div className="rounded-2xl border border-[var(--color-gray-200)] bg-white p-4 sm:p-5">
      ...
      <div className="flex gap-3 overflow-x-auto snap-x snap-mandatory scrollbar-hide">
        {orders.map((order) => (
          <div
            key={order.id}
            className="flex-none w-56 snap-start rounded-xl border border-gray-100 bg-gray-50 p-3"
          >
            <div className="flex items-center justify-between">
              <p className="text-xs font-bold text-gray-900">#{order.id.slice(0, 8)}</p>
              <p className="text-xs text-gray-500">{formatCad(order.totalAmount)}</p>
            </div>
            ...
            <button
              ...
              className="mt-2 w-full rounded-full border border-gray-300 py-1.5 text-[11px] font-semibold text-gray-700 transition-colors hover:border-gray-900 hover:text-gray-900 disabled:opacity-50"
            >
