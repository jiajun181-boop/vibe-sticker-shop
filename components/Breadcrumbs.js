  ...
  return (
    <nav aria-label="Breadcrumb" className={`mb-4 text-xs ${dark ? "text-gray-400" : "text-[var(--color-gray-400)]"}`}>
      <ol className="flex items-center gap-1.5 flex-wrap">
        <li>
          <Link href="/" className={`transition-colors ${dark ? "hover:text-[#fff]" : "hover:text-[var(--color-gray-700)]"}`}>{t("nav.home")}</Link>
        </li>
        {items.map((item, i) => (
          <li key={i} className="flex items-center gap-1.5">
            <span className={dark ? "text-gray-600" : "text-[var(--color-gray-300)]"}>/</span>
            {item.href ? (
              <Link href={item.href} className={`transition-colors ${dark ? "hover:text-[#fff]" : "hover:text-[var(--color-gray-700)]"}`}>{item.label}</Link>
            ) : (
