      ...
      <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
        {items.map((item) => {
          ...
          <Link
            ...
            className="flex-shrink-0 w-40 overflow-hidden rounded-2xl border border-[var(--color-gray-200)] bg-white transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md"
          >
            ...
            <div className="p-3">
              ...
              {item.basePrice > 0 && (
                <p className="mt-0.5 text-[11px] text-[var(--color-gray-500)]">{t("product.from", { price: formatCad(item.basePrice) })}</p>
              )}
            </div>
          </Link>
          );
