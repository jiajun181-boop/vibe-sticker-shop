...
export default function ProductSpecsSection({ specs, productSpecs, t }) {
  return (
    <>
      {/* Quality Guarantee */}
      <section className="rounded-2xl border border-[var(--color-gray-200)] bg-white p-6 sm:p-8">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            { icon: "M11.42 15.17l-5.59-5.17a8.002 8.002 0 0111.77-1.01l.17.17M21 12a9 9 0 11-18 0 9 9 0 0118 0z", title: t("product.qgCustom"), desc: <Link href="/quote" className="text-[var(--color-gray-900)] underline underline-offset-2 hover:text-black">{t("product.qgGetQuote")}</Link> },
          ].map((item) => (
            <div key={item.title} className="flex gap-3">
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs text-[var(--color-gray-500)]">{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="rounded-2xl border border-[var(--color-gray-200)] bg-white p-6 sm:p-8">
        ...
        <p className="mt-1 text-xs text-[var(--color-gray-500)]">{t("product.printBasicsSubtitle")}</p>
        <dl className="mt-4 grid gap-x-6 gap-y-3 sm:grid-cols-2 lg:grid-cols-3">
          {[
            ...
            <div key={term} className="flex gap-2">
              <dt className="text-xs font-semibold text-[var(--color-gray-900)] whitespace-nowrap">{term}</dt>
              <dd className="text-xs text-[var(--color-gray-500)]">{def}</dd>
            </div>
          ))}
        </dl>
      </section>
