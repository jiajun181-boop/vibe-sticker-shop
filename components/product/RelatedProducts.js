          ...
          return (
            <Link key={item.id} href={`/shop/${item.category}/${item.slug}`} className="overflow-hidden rounded-2xl border border-[var(--color-gray-200)] bg-white transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md">
              <div className="relative aspect-square sm:aspect-[4/3] bg-[var(--color-gray-100)]">
                <Image src={relatedImage} alt={item.images?.[0]?.alt || item.name} fill className="object-cover" sizes="(max-width: 640px) 50vw, (max-width: 1024px) 50vw, 25vw" unoptimized={isSvgImage(relatedImage)} />
              </div>
