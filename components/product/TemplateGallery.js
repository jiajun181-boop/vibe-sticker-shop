...
export default function TemplateGallery({ templates }) {
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-5">
      ...
      <p className="mt-1 text-xs text-gray-500">{t("bc.templatesHint")}</p>
      ...
      <div className="mt-3 grid grid-cols-2 gap-3">
        {templates.map((tmpl, i) => (
          <button
            ...
            className="group relative overflow-hidden rounded-xl border border-gray-200 bg-gray-50 transition-all hover:border-gray-400 hover:shadow-sm"
          >
              ...
              {tmpl.url ? (
                ...
                <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200">
                  <div className="text-center">
                    ...
                    <p className="mt-1 text-[10px] text-gray-400">{tmpl.alt || `Template ${i + 1}`}</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="aspect-[3.5/2] relative w-[min(600px,90vw)]">
              {templates[selected].url ? (
                ...
                <div className="flex h-full w-full items-center justify-center bg-gray-100">
                  <p className="text-sm text-gray-500">{templates[selected].alt || "Template preview"}</p>
                </div>
              )}
            </div>
          </div>
