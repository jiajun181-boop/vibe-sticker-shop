...
/** Print-only discount rate for display stands (banner without hardware) */
export const PRINT_ONLY_DISCOUNT_RATE = 0.35; // 35%
...
/** Which payment statuses a given payment status can transition to */
...
/** Which production statuses a given production status can transition to */
export const VALID_PRODUCTION_TRANSITIONS = {
  not_started:    ["preflight", "in_production", "on_hold", "canceled"],
  preflight:      ["in_production", "on_hold", "canceled"],
  in_production:  ["ready_to_ship", "on_hold", "canceled"],
  ready_to_ship:  ["shipped", "on_hold"],
  shipped:        ["completed"],
  completed:      [],                           // terminal
  on_hold:        ["not_started", "preflight", "in_production", "ready_to_ship", "canceled"],
  canceled:       ["not_started"],              // can reopen
};

// ─── Production job transitions ──────────────────────────────────
/** Which job statuses a given job status can transition to */
export const VALID_JOB_TRANSITIONS = {
  queued:        ["assigned", "on_hold", "canceled"],
  assigned:      ["printing", "queued", "on_hold", "canceled"],
  printing:      ["quality_check", "finished", "on_hold", "canceled"],
  quality_check: ["finished", "printing", "on_hold", "canceled"],   // printing = rework
  finished:      ["shipped", "on_hold"],
  shipped:       [],                                     // terminal
  on_hold:       ["queued", "assigned", "printing", "quality_check", "finished", "canceled"],
  canceled:      ["queued"],                             // can reopen a canceled job
};

// ─── Invoice status transitions ─────────────────────────────────
export const VALID_INVOICE_STATUSES = ["draft", "sent", "paid", "overdue", "void"];

/** Which invoice statuses a given invoice status can transition to */
...
// ─── Governance: valid change types ─────────────────────────────
/** All recognized changeType values for the approval / change-log system */
export const VALID_CHANGE_TYPES = [
  "preset_create", "preset_config_edit", "preset_delete", "preset_assign",
  "material_create", "material_delete", "material_cost_edit",
  "b2b_discount", "b2b_delete", "b2b_floor_override",
  "vendor_cost_create", "vendor_cost_update", "vendor_cost_delete",
  "bulk_price_update",
];
