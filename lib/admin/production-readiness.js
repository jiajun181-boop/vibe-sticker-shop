...
export function assessItem(item, orderId) {
  ...
  // --- Print-and-cut detection ---
  const isPrintAndCut = (family === "sticker" || family === "label");
  const contourReady = !!(meta.contourSvg || meta.bleedMm);
  // contourAppliedAt OR contourToolJobAt — writeback historically used either name
  const registrationMarkReady = contourReady && !!(meta.contourAppliedAt || meta.contourToolJobAt);
  // contourConfidence is a string: "good" | "rectangular" | "low" (not a number)
  const confidenceNeedsReview = meta.contourConfidence === "low" || meta.contourConfidence === "rectangular";
  const shapeNeedsReview = meta.contourShapeType === "near-rectangular" || meta.contourShapeType === "rectangular";
  const manualReviewRequired = isPrintAndCut && contourReady && (confidenceNeedsReview || shapeNeedsReview);

  // --- Check production job status first ---
  const job = item.productionJob;
  if (job) {
    if (job.status === "shipped" || job.status === "finished") {
      return { level: READINESS.DONE, family, reasons: [], nextAction: null, toolLink: null, isPrintAndCut, contourReady, registrationMarkReady, manualReviewRequired: false };
    }
    if (["printing", "quality_check", "assigned"].includes(job.status)) {
      return {
        ...
        nextAction: null,
        toolLink: null,
        isPrintAndCut, contourReady, registrationMarkReady, manualReviewRequired: false,
      };
    }
  }
  ...
  if (!hasArt) {
    ...
    } else if (artStatus === "provided") {
      reasons.push({
        code: "artwork_provided",
        message: "Artwork confirmed off-platform — awaiting upload to system",
        severity: "warning",
      });
      nextAction = "admin.readiness.actionAwaitArtwork";
  ...
  // --- Manual review for complex contours ---
  if (manualReviewRequired) {
    reasons.push({
      code: "manual_review_needed",
      message: "Manual review — complex contour shape or low confidence score",
      severity: "warning",
    });
  }
  ...
  return { level, family, reasons, nextAction, toolLink, isPrintAndCut, contourReady, registrationMarkReady, manualReviewRequired };
}
...
export function assessOrder(order) {
  ...
  // Filter out service-fee items (e.g. design help) — they are not manufacturable
  ...
  if (order.productionStatus === "shipped" || order.productionStatus === "completed") {
    return {
      ...
      items: productionItems.map((item) => assessItem(item, order.id)),
      blockerCount: 0, warningCount: 0, readyCount: productionItems.length,
  ...
  const items = productionItems.map((item) => assessItem(item, order.id));
  let blockerCount = 0;
  let warningCount = 0;
  let readyCount = 0;
...
export function categorizeForTaskQueue(orders) {
  ...
  for (const order of orders) {
    ...
    } else if (codes.has("needs_contour") || codes.has("manual_review_needed")) {
...
export function assessOrderPackage(order) {
  // Filter out service-fee items — they have no production files
...
// ── Executable action mapper ─────────────────────────────────────────────────

/**
 * Map an item's readiness assessment to a concrete, executable action.
 *
 * Returns the single most urgent action for the item, or null if no action is needed.
 *
 * @param {object} assessment — return value of assessItem()
 * @param {string} orderId
 * @param {string} itemId
 * @returns {{ action: string, actionType: string, toolLink: string|null, reasonCode: string, severity: string, totalIssueCount: number }|null}
 */
export function getExecutableAction(assessment, orderId, itemId) {
  if (!assessment || assessment.level === READINESS.DONE || assessment.level === READINESS.READY) {
    return null;
  }
  ...
  // Find the worst reason (blockers first, then warnings)
  const worst = reasons.find((r) => r.severity === "blocker") || reasons.find((r) => r.severity === "warning") || reasons[0];

  const params = [orderId && `orderId=${orderId}`, itemId && `itemId=${itemId}`].filter(Boolean).join("&");

  const ACTION_MAP = {
    no_artwork:            { action: "Contact customer for artwork upload", actionType: "contact", toolLink: null },
    artwork_url_missing:   { action: "Re-upload artwork — file name present but URL missing", actionType: "upload", toolLink: null },
    artwork_provided:      { action: "Artwork confirmed off-platform — upload to system when received", actionType: "upload", toolLink: null },
    upload_later:          { action: "Follow up with customer to collect artwork", actionType: "contact", toolLink: null },
    design_help:           { action: "Prepare artwork for customer ($45 design help)", actionType: "design", toolLink: null },
    needs_contour:         { action: "Run contour tool to generate die-cut path", actionType: "tool", toolLink: `/admin/tools/contour${params ? `?${params}` : ""}` },
    manual_review_needed:  { action: "Manual review — complex contour shape or low confidence", actionType: "review", toolLink: `/admin/tools/contour${params ? `?${params}` : ""}` },
    white_ink_pending:     { action: "Process white ink layer file", actionType: "process", toolLink: null },
    stamp_no_preview:      { action: "Open Stamp Studio to generate preview", actionType: "tool", toolLink: `/admin/tools/stamp-studio${params ? `?${params}` : ""}` },
    missing_dimensions:    { action: "Confirm dimensions with customer", actionType: "contact", toolLink: null },
    missing_material:      { action: "Confirm material/substrate with customer", actionType: "contact", toolLink: null },
    rush:                  { action: "Rush order — prioritize immediately", actionType: "prioritize", toolLink: null },
    two_sided_single_file: { action: "Verify if front+back are combined, or follow up for back file", actionType: "verify", toolLink: null },
    booklet_no_pages:      { action: "Confirm page count with customer", actionType: "contact", toolLink: null },
    booklet_no_binding:    { action: "Confirm binding type with customer", actionType: "contact", toolLink: null },
    ncr_no_parts:          { action: "Confirm form type / part count with customer", actionType: "contact", toolLink: null },
    vehicle_no_type:       { action: "Confirm vehicle graphic type with customer", actionType: "contact", toolLink: null },
    vehicle_no_body:       { action: "Confirm vehicle make/model with customer", actionType: "contact", toolLink: null },
    vehicle_dot_no_text:   { action: "Confirm DOT/compliance text content with customer", actionType: "contact", toolLink: null },
    banner_no_finishing:   { action: "Confirm finishing (grommets/hemming/pockets) with customer", actionType: "contact", toolLink: null },
    sign_no_hardware:      { action: "Confirm if customer needs stakes/frames for yard sign", actionType: "contact", toolLink: null },
    biz_card_missing_names:{ action: "Follow up for remaining name variation artwork files", actionType: "contact", toolLink: null },
    in_production:         { action: "In production — monitor progress", actionType: "monitor", toolLink: null },
  };

  const mapped = ACTION_MAP[worst.code];
  if (!mapped) {
    // Fallback for unknown reason codes
    return {
      action: worst.message,
      actionType: "unknown",
      toolLink: null,
      reasonCode: worst.code,
      severity: worst.severity,
      totalIssueCount: reasons.length,
    };
  }
