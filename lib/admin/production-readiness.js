...
export function assessItem(item, orderId) {
  ...
  if (!hasArt) {
    if (artStatus === "missing" || artStatus === "file-name-only") {
      ...
      if (family === "stamp") {
        ...
        toolLink = { label: "admin.readiness.toolStamp", href: `/admin/tools/stamp-studio${orderId ? `?orderId=${orderId}` : ""}` };
      } else {
  ...
  if ((family === "sticker" || family === "label") && !meta.contourSvg && !meta.bleedMm) {
    ...
    if (!nextAction) {
      ...
      toolLink = { label: "admin.readiness.toolContour", href: `/admin/tools/contour${orderId ? `?orderId=${orderId}` : ""}` };
    }
  }
