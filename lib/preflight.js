...
export function preflightItem(item) {
  ...
  // Service-fee rows are financial line items (e.g. "Design Help Service"),
  // not producible goods — skip all preflight checks.
  if (meta.isServiceFee === "true") return issues;
  ...
  // Artwork provided off-platform (admin manual order intent) — not missing, just not uploaded to system
  if (artworkIntent === "provided") {
    issues.push({
      level: "info",
      code: "artwork_provided",
      message: "Artwork confirmed provided off-platform — no upload needed",
    });
  }

  // 1. No artwork uploaded (skip if customer explicitly chose "upload later", design-help, or admin confirmed "provided")
  if (!meta.fileName && !meta.artworkUrl && !meta.fileUrl && !meta.stampPreviewUrl && artworkIntent !== "upload-later" && artworkIntent !== "provided" && !designHelp) {
    issues.push({
      level: "warning",
      code: "no_artwork",
  ...
  const isSign = family === "sign" || family === "banner" || family === "surface";
  ...
  if ((family === "sign" || family === "surface") && (name.includes("window") || name.includes("perforated"))) {
 ...
 * @returns {"sticker"|"label"|"stamp"|"canvas"|"banner"|"sign"|"surface"|"booklet"|"ncr"|"business-card"|"vehicle"|"standard-print"|"other"}
 */
export function detectProductFamily(item) {
  ...
  // Window/wall/floor products are their own family — NOT "sign"
  if (name.includes("wall") || name.includes("floor graphic") || name.includes("floor decal")
    || name.includes("window film") || name.includes("window cling") || name.includes("window decal")
    || name.includes("perforated vinyl") || name.includes("frosted film") || name.includes("one-way vision")
    || name.includes("blockout") || name.includes("glass waistline")) return "surface";
  if (name.includes("vinyl lettering")) return "label";
  if (name.includes("a-frame") || name.includes("shelf") || name.includes("magnetic sign")) return "sign";
  // Generic "window" / "wall" / "floor" still catch surface products
  if (name.includes("window") || name.includes("wall graphic") || name.includes("wall mural")) return "surface";
...
export function buildSpecsSummary(item) {
  ...
  if (family === "banner" || family === "sign" || family === "surface") {
