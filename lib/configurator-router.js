...
import { getCanvasTypeForSlug } from "./canvas-order-config";
...
import { isBusinessCardSlug } from "./business-card-configs";
import { isStampSlug } from "./stamp-order-config";
 ...
 *
 * component values: "stickers" | "booklets" | "ncr" | "banners" | "signs" | "vehicle" | "canvas" | "surfaces" | "marketing-print" | "business-cards"
 */
export function getConfiguratorForSlug(slug) {
  // Business cards have dedicated /order/ pages — return the slug itself
  if (isBusinessCardSlug(slug)) return { component: "business-cards", defaultValue: slug };
  ...
  const canvas = getCanvasTypeForSlug(slug);
  if (canvas) return { component: "canvas", defaultValue: canvas };

  const surface = getSurfaceTypeForSlug(slug);
  if (surface) return { component: "surfaces", defaultValue: surface };

  if (isStampSlug(slug)) return { component: "stamps", defaultValue: slug };
