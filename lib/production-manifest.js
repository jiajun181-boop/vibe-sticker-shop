...
export function buildProductionManifest(order) {
  ...
  const items = (order.items || [])
    ...
    .map((item) => buildItemManifest(item, order.id));
  ...
  return {
    ...
    isRush,
    isManualOrder: orderTags.includes("admin_manual"),
    items,
...
function buildItemManifest(item, orderId) {
  ...
  // Artwork intake state — tells production what to expect
  const artworkStatus = getArtworkStatus(item);
  ...
  return {
    ...
    artworkStatus,         // "uploaded" | "provided" | "design-help" | "upload-later" | "file-name-only" | "missing"
    artworkIntent,         // "provided" | "upload-later" | "design-help" | null
    isDesignHelp: meta.designHelp === true || meta.designHelp === "true" || artworkIntent === "design-help",

    // Pricing provenance (for operator reference on manual orders)
    pricingSource: meta.pricingSource || null,    // "manual" | "calculated" | "overridden"
    pricingQuoteSlug: meta.pricingQuoteSlug || null,
...
export function manifestToText(manifest) {
  ...
  const lines = [
    ...
    `Source: ${manifest.isManualOrder ? "Manual (admin)" : "Storefront"}`,
  ...
  for (const item of manifest.items) {
    ...
    if (item.artworkStatus && item.artworkStatus !== "uploaded") {
      lines.push(`  Artwork: ${item.artworkStatus.toUpperCase()}${item.isDesignHelp ? " (needs designer)" : ""}`);
    }
    if (item.pricingSource && item.pricingSource !== "calculated") {
      lines.push(`  Pricing: ${item.pricingSource}`);
    }
