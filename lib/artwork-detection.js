...
// ---------------------------------------------------------------------------
// Customer/admin-facing per-item helpers
// Keep these aligned with the TS contract used by route tests.
// ---------------------------------------------------------------------------

export function itemHasArtwork(item) {
  if (isServiceFeeItem(item)) return false;
  return hasArtworkUrl(item);
}

export function itemIsDesignHelp(item) {
  if (isServiceFeeItem(item)) return false;
  const meta = parseMeta(item);
  return meta.artworkIntent === "design-help" || meta.designHelp === true || meta.designHelp === "true";
}

export function itemIsArtworkProvided(item) {
  if (isServiceFeeItem(item)) return false;
  const meta = parseMeta(item);
  return meta.artworkIntent === "provided";
}

export function itemNeedsArtwork(item) {
  if (isServiceFeeItem(item)) return false;
