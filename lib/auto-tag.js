...
export function computeAutoTags(order) {
  ...
  for (const item of items) {
    const meta = item.meta && typeof item.meta === "object" ? item.meta : {};
    if (meta.isServiceFee === "true") continue;
  ...
  // Missing artwork (needs follow-up) — uses same detection as customer/admin surfaces
  // Note: imported dynamically to keep this file isomorphic (works in both JS and TS contexts)
  for (const item of items) {
    const meta = item.meta && typeof item.meta === "object" ? item.meta : {};
    ...
    const isDesignHelp = meta.artworkIntent === "design-help" || meta.designHelp === true || meta.designHelp === "true";
    const isProvided = meta.artworkIntent === "provided"; // admin confirmed off-platform
    if (!hasFile && !isDesignHelp && !isProvided) {
...
/**
 * Refresh auto-tags for an order: recalculates from scratch and removes
 * stale auto-tags (like "missing-artwork" after file upload).
 * Non-blocking — never throws.
 * @param {string} orderId
 * @param {import("@prisma/client").PrismaClient} prisma
 */
export async function refreshAutoTags(orderId, prisma) {
  try {
    const order = await prisma.order.findUnique({
      where: { id: orderId },
      include: { items: true },
    ...
    const freshTags = computeAutoTags(order);

    // Tags that can be auto-removed when conditions change
    const REMOVABLE_TAGS = ["missing-artwork", "upload-later"];

    const currentTags = order.tags || [];
    // Keep manually-added tags (non-auto) + fresh auto-tags
    const manualTags = currentTags.filter(
      (t) => !REMOVABLE_TAGS.includes(t) || freshTags.includes(t)
    );
    const merged = [...new Set([...manualTags, ...freshTags])];

    // Only update if changed
    if (merged.length === currentTags.length && merged.every((t) => currentTags.includes(t))) return;
    ...
    console.error("[AutoTag Refresh] Failed:", err);
  }
}
