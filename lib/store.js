...
import { HST_RATE, FREE_SHIPPING_THRESHOLD, SHIPPING_OPTIONS, DESIGN_HELP_CENTS, MAX_ITEM_QUANTITY } from "@/lib/order-config";
    ...
    (set, get) => ({
      ...
      addItem: (item) => {
        ...
        if (existing && !forceNewLine) {
          set({
            cart: cart.map((c) =>
              key(c, cleanedItem) ? { ...c, quantity: Math.min(MAX_ITEM_QUANTITY, c.quantity + cleanedItem.quantity) } : c
            ),
          });
        } else {
          const _cartId =
      ...
      updateQuantity: (_cartId, qty) =>
        set((s) => ({
          cart: s.cart.map((c) =>
            c._cartId === _cartId ? { ...c, quantity: Math.min(MAX_ITEM_QUANTITY, Math.max(1, qty)) } : c
          ),
        })),
      ...
      getSubtotal: () => {
        ...
        return cart.reduce((sum, item) => {
          ...
          const designHelp = (item.options?.designHelp === true || item.options?.designHelp === "true")
            ? DESIGN_HELP_CENTS : 0;
          return sum + lineTotal + designHelp;
        }, 0);
      },
      getShippingCost: () => {
        const { shipping } = get();
