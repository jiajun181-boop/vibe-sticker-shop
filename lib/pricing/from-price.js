...
export function computeFromPrice(product) {
  ...
  try {
    ...
    if (model === "QTY_TIERED") {
      ...
      // Use product-level costPlusDefaults for material/printMode, smallest size 24×36
      const defaults = product.optionsConfig?.costPlusDefaults || {};
      const w = Number(product.optionsConfig?.displayMinSize?.widthIn) || product.minWidthIn || 24;
      const h = Number(product.optionsConfig?.displayMinSize?.heightIn) || product.minHeightIn || 36;
      body = {
        slug: product.slug,
        quantity: 1,
        widthIn: w,
        ...
        cutType: defaults.cutType || "rectangular",
      };
    } else {
      return product.basePrice || 0;
    }
