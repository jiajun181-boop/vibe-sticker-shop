...
function roundUp99(dollars) {
  ...
  if (!Number.isFinite(dollars) || dollars < 1) return 0.99;
...
const MARGIN_TIERS = {
  ...
  print: [
    [24, 0.75], [99, 0.75], [499, 0.70], [999, 0.65], [4999, 0.60], [Infinity, 0.45],
  ],
  canvas: [
...
export function getMargin(category, qty) {
  ...
  for (const [maxQty, margin] of tiers) {
    if (qty <= maxQty) { m = margin; break; }
    m = margin;
  }
  // Safety cap: margin >= 1.0 would cause division by zero in cost / (1 - margin)
  return Math.min(m, 0.95);
}
...
// ── Minimum prices (defaults — overridable via Settings DB) ──────
...
function getBannerSetupFee(qty, settings) {
  if (settings) {
    const s1 = settings["pricing.banner.setup1_2"];
    const s2 = settings["pricing.banner.setup3_5"];
    const s3 = settings["pricing.banner.setup6plus"];
    if (s1 != null || s2 != null || s3 != null) {
      if (qty <= 2) return s1 ?? 28;
      if (qty <= 5) return s2 ?? 15;
      return s3 ?? 10;
    }
  }
...
function getMaterialMarkup(materialName, settings) {
  const holoMult = settings?.["pricing.material.holographic"] ?? MATERIAL_MARKUP.holographic;
  const lower = (materialName || '').toLowerCase();
  if (lower.includes('holographic')) return holoMult;
  return 1.0;
}
...
// ═══════════════════════════════════════════════════════════════════
export function vinylPrint({ widthIn, heightIn, quantity, material, inkRate, lamination, options, marginCategory, settings }) {
  ...
  const materialMarkup = getMaterialMarkup(material.name, settings);
  let price = rawPrice * materialMarkup;

  // Apply optional surcharges from sticker configurator
  const shapeSurcharge = Number(options?.shapeSurcharge) || 0;
  if (shapeSurcharge > 0) price *= (1 + shapeSurcharge);

  const turnaroundMult = Number(options?.turnaroundMultiplier) || 0;
  if (turnaroundMult > 1) price *= turnaroundMult;

  // Print mode multipliers (white ink on transparent materials) — overridable via settings
  const whiteColorMult = settings?.["pricing.ink.whiteColor"] ?? 1.30;
  const colorWhiteColorMult = settings?.["pricing.ink.colorWhiteColor"] ?? 1.60;
  const printModeMultipliers = { color_only: 1.0, white_only: 1.0, white_color: whiteColorMult, color_white_color: colorWhiteColorMult };
  const pm = options?.printMode;
  if (pm && printModeMultipliers[pm] && printModeMultipliers[pm] !== 1.0) {
    price *= printModeMultipliers[pm];
  }

  const stickerMin = settings?.["pricing.minPrice.stickers"] ?? MIN_PRICE_STICKERS;
  const generalMin = settings?.["pricing.minPrice.general"] ?? MIN_PRICE_GENERAL;
  const minPrice = (cat === 'stickers' || cat === 'wwf') ? stickerMin : generalMin;
  ...
  return {
    ...
    meta: {
      ...
      shapeSurcharge: shapeSurcharge > 0 ? shapeSurcharge : undefined,
      turnaroundMultiplier: turnaroundMult > 1 ? turnaroundMult : undefined,
      printMode: pm || undefined,
      marginCategory: cat,
    },
  };
}
...
// ═══════════════════════════════════════════════════════════════════
export function boardSign({ widthIn, heightIn, quantity, boardMaterial, vinylMaterial, inkRate, options, marginCategory, settings }) {
  const w = Number(widthIn);
  const h = Number(heightIn);
  const qty = Number(quantity);
  ...
  // Labor — overridable via settings
  const boardCutRate = settings?.["pricing.labor.boardCut"] ?? 0.50;
  const faceSmallRate = settings?.["pricing.labor.faceSmall"] ?? 0.50;
  const faceMediumRate = settings?.["pricing.labor.faceMedium"] ?? 0.75;
  const faceLargeRate = settings?.["pricing.labor.faceLarge"] ?? 1.00;

  const cutLabor = boardCutRate * qty;
  let faceLabor;
  if (areaSqft <= 2) faceLabor = faceSmallRate;
  else if (areaSqft <= 6) faceLabor = faceMediumRate;
  else faceLabor = faceLargeRate;
  ...
  const generalMin = settings?.["pricing.minPrice.general"] ?? MIN_PRICE_GENERAL;
  const finalPrice = Math.max(roundUp99(price), generalMin);
...
// TEMPLATE C: banner
// ═══════════════════════════════════════════════════════════════════
export function banner({ widthIn, heightIn, quantity, material, inkRate, finishing, accessories, marginCategory, settings }) {
  const w = Number(widthIn);
  const h = Number(heightIn);
  const qty = Number(quantity);
  const areaSqft = (w * h) / 144;
  ...
  if (finishing) {
    ...
    // Pole pockets — overridable via settings
    if (finishing.polePockets) {
      const ppRate = settings?.["pricing.banner.polePocket"] ?? 0.50;
      const ppCost = ppRate * qty;
      finishingCost += ppCost;
      finishingDetails.push({ name: 'Pole Pockets', cost: ppCost });
    }
    // Wind slits — overridable via settings
    if (finishing.windSlits) {
      const wsRate = settings?.["pricing.banner.windSlit"] ?? 0.25;
      const wsCost = wsRate * qty;
  ...
  if (Array.isArray(accessories)) {
    for (const acc of accessories) {
      ...
      const accMarkup = settings?.["pricing.accessories.markup"] ?? ACCESSORY_MARKUP;
      const marked = cost * accMarkup;
      accessoryPrice += marked;
      accessoryDetails.push({ name: acc.name, cost, sellingPrice: marked });
    }
  }

  // Setup fee (flat, not margined) — overridable via settings
  const setupFee = getBannerSetupFee(qty, settings);
  ...
  return {
    ...
    meta: {
      ...
      finishingDetails,
      accessoryDetails,
      accessoryMarkup: settings?.["pricing.accessories.markup"] ?? ACCESSORY_MARKUP,
      setupFee,
      marginCategory: cat,
    },
  };
...
// ═══════════════════════════════════════════════════════════════════
export function paperPrint({ widthIn, heightIn, quantity, paper, inkCostPerClick, options, lamination, marginCategory, settings }) {
  const w = Number(widthIn);
  const h = Number(heightIn);
  const qty = Number(quantity);
  ...
  const isOversize = bleedW > parentW || bleedH > parentH;
  const piecesPerSheet = isOversize ? 1 : impositionCount(parentW, parentH, bleedW, bleedH);
  ...
  let paperCost;
  if (isOversize) {
    // Large format (posters etc.): scale cost proportionally to actual area
    const parentSqIn = parentW * parentH;
    const pieceSqIn = bleedW * bleedH;
    paperCost = qty * sheetCost * (pieceSqIn / parentSqIn);
  } else {
    paperCost = sheetsNeeded * sheetCost;
  }

  // Ink cost: per piece (not per sheet) — each piece is a click
  const passes = options?.doubleSided ? 2 : 1;
  // For oversize pieces, scale ink cost by area relative to a standard sheet
  const inkCost = isOversize
    ? qty * inkClick * passes * ((w * h) / (parentW * parentH))
    : qty * inkClick * passes;

  // Lamination (optional)
  const lamSqft = isOversize
    ? (bleedW * bleedH / 144) * qty * passes
    : (parentW * parentH / 144) * sheetsNeeded * passes;
  const lamCost = lamination?.costPerSqft
    ? (Number(lamination.costPerSqft) * lamSqft)
    : 0;

  // Finishing costs — all rates overridable via settings
  let finishingCost = 0;
  if (options?.scoring) {
    finishingCost += qty * (settings?.["pricing.labor.scoring"] ?? 0.01);
  }
  if (options?.binding === 'saddle_stitch') {
    finishingCost += qty * (settings?.["pricing.labor.saddleStitch"] ?? 0.15);
  } else if (options?.binding === 'perfect_bind') {
    finishingCost += qty * (settings?.["pricing.labor.perfectBind"] ?? 0.50);
  } else if (options?.binding === 'coil') {
    finishingCost += qty * (settings?.["pricing.labor.coilBind"] ?? 0.75);
  }
  if (options?.roundedCorners) {
    finishingCost += qty * (settings?.["pricing.labor.roundedCorner"] ?? 0.02) * 4; // 4 corners
  }
  if (options?.holePunch) {
    finishingCost += qty * (settings?.["pricing.labor.holePunch"] ?? 0.01);
  }
  if (options?.folds) {
    finishingCost += qty * (settings?.["pricing.labor.fold"] ?? 0.01) * Number(options.folds);
  }

  let totalCost = paperCost + inkCost + lamCost + finishingCost;
  // Double-sided surcharge — overridable via settings
  if (options?.doubleSided) {
    totalCost *= (settings?.["pricing.surcharge.doubleSided"] ?? 1.20);
  }
  const margin = getMargin(cat, qty);
  const price = totalCost / (1 - margin);
  const generalMin = settings?.["pricing.minPrice.general"] ?? MIN_PRICE_GENERAL;
...
// TEMPLATE E: canvas
// ═══════════════════════════════════════════════════════════════════
export function canvas({ widthIn, heightIn, quantity, canvasMaterial, inkRate, options, marginCategory, settings }) {
  const w = Number(widthIn);
  const h = Number(heightIn);
  const qty = Number(quantity);
  const areaSqft = (w * h) / 144;
  ...
  const margin = getMargin(cat, qty);
  const price = totalCost / (1 - margin);
  const canvasMin = settings?.["pricing.minPrice.canvas"] ?? MIN_PRICE_CANVAS;
  const finalPrice = Math.max(roundUp99(price), canvasMin);
...
// ═══════════════════════════════════════════════════════════════════
export function vinylCut({ widthIn, heightIn, quantity, material, marginCategory, settings }) {
  const w = Number(widthIn);
  const h = Number(heightIn);
  const qty = Number(quantity);
  const areaSqft = (w * h) / 144;
