...
const STICKER_SETUP_FEE_CENTS = 1200; // $12 CAD
 ...
 */
function calculateStickerRefPrice(input) {
  ...
  const totalCents = refCents + STICKER_SETUP_FEE_CENTS;
  ...
  return {
    ...
    breakdown: {
      ...
      setupFee: STICKER_SETUP_FEE_CENTS,
...
export async function calculatePrice(product, input) {
  ...
  switch (template) {
    ...
    case "vinyl_print": {
      ...
      if (input.options?.isSticker && !SLUG_SYNTHETIC_MATERIAL[product.slug]) {
        return calculateStickerRefPrice(input);
      }

      const material = SLUG_SYNTHETIC_MATERIAL[product.slug] || await findMaterial(materialId);
