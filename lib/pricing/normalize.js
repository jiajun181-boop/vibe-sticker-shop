...
export function normalizeInput(body) {
  ...
  // Print mode (optional, for COST_PLUS)
  if (body.printMode) input.printMode = String(body.printMode);

  // Cut type (optional, for COST_PLUS)
  if (body.cutType) input.cutType = String(body.cutType);

  // B2B flag (optional, for COST_PLUS and QTY_TIERED wholesale pricing)
  if (body.isB2B != null) input.isB2B = !!body.isB2B;

  // Panel multiplier (optional, for canvas split-panel pricing)
  if (body.panelMultiplier != null) input.panelMultiplier = Number(body.panelMultiplier);
  ...
  // Names (optional, for multi-name business card orders)
  if (body.names != null) {
    const n = Math.floor(Number(body.names));
    if (n >= 1 && n <= 100) input.names = n;
  }
