...
export function compute(config, input) {
  ...
  // B2B / wholesale pricing — use b2bTiers if caller is B2B and preset defines them
  const tierSource = input.isB2B && config.b2bTiers?.length ? config.b2bTiers : config.tiers;

  // Tiers sorted ascending by minQty
  const tiers = [...(tierSource || [])].sort(
  ...
  const isB2B = !!(input.isB2B && config.b2bTiers?.length);
  const breakdown = [
    {
      label: `${billableQty} pcs @ $${(unitPriceDollars * matMul).toFixed(2)}/ea${isB2B ? " (wholesale)" : ""}`,
  ...
  return {
    ...
    meta: {
      ...
      ...(input.isB2B && config.b2bTiers?.length ? { isB2B: true } : {}),
    },
  };
}
