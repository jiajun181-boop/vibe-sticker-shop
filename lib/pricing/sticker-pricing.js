...
export const VINYL_MATERIALS = [
  ...
  { id: "heavy_duty_vinyl", group: "vinyl",   label: "Heavy Duty Vinyl", surchargeLabel: "+15%", surchargePercent: 15 },
...
export const MATERIAL_COMPAT = {
  ...
  frosted_vinyl:         { die_cut: true, kiss_cut: true, sheets: true, vinyl_lettering: false },
  heavy_duty_vinyl:      { die_cut: true, kiss_cut: true, sheets: true, vinyl_lettering: false },
 ...
 */
/** Custom shape surcharge (+15%) */
export const CUSTOM_SHAPE_SURCHARGE = 0.15;

/** Print mode multipliers */
export const PRINT_MODE_MULTIPLIERS = {
  color_only: 1.0,
  white_only: 1.0,
  white_color: 1.30,
  color_white_color: 1.60,
};

/** Lamination multipliers (aligned to 1.10x for consistency with actual cost) */
export const LAMINATION_MULTIPLIERS = {
  none: 1.0,
  gloss: 1.10,
  matte: 1.10,
};
 ...
 * @param {number}  [opts.shapeSurcharge] - shape surcharge multiplier (e.g. 0.15 for +15%)
 * @param {number}  [opts.printModeMultiplier] - white ink print mode multiplier (default 1.0)
 * @param {number}  [opts.laminationMultiplier] - lamination multiplier (default 1.0)
 * @param {number}  [opts.turnaroundMultiplier] - rush/standard multiplier (default 1.0)
...
export function calcVinylPrice(width, height, qty, material, cutType, opts = {}) {
  ...
  if (opts.shapeSurcharge) {
    totalPrice *= (1 + opts.shapeSurcharge);
  }

  // Print mode multiplier (white ink options)
  if (opts.printModeMultiplier && opts.printModeMultiplier !== 1.0) {
    totalPrice *= opts.printModeMultiplier;
  }

  // Lamination multiplier
  if (opts.laminationMultiplier && opts.laminationMultiplier !== 1.0) {
    totalPrice *= opts.laminationMultiplier;
  }

  // Turnaround multiplier
  if (opts.turnaroundMultiplier && opts.turnaroundMultiplier !== 1.0) {
    totalPrice *= opts.turnaroundMultiplier;
  }
...
// Customer delivery sheet sizes — printed on 12×18, then cut down
export const CUSTOMER_SHEET_SIZES = [
  { id: "4x6", w: 4, h: 6, label: '4" × 6"', desc: "Compact", perProduction: 9 },
  { id: "6x9", w: 6, h: 9, label: '6" × 9"', desc: "Standard", perProduction: 4 },
];
export const CUTTING_FEE_PER_SHEET = 0.25; // $0.25 per customer sheet

/**
 * Calculate how many stickers fit on one sheet.
 ...
 * @param {number} [sheetW] - sheet width (default 12")
 * @param {number} [sheetH] - sheet height (default 18")
 */
export function stickersPerSheet(w, h, shape = "rect", sheetW = SHEET_USABLE_W, sheetH = SHEET_USABLE_H) {
  const gap = shape === "rect" ? 0 : STICKER_GAP_INCHES;

  const cols1 = Math.floor(sheetW / (w + gap));
  const rows1 = Math.floor(sheetH / (h + gap));
  // Try rotated
  const cols2 = Math.floor(sheetW / (h + gap));
  const rows2 = Math.floor(sheetH / (w + gap));
 ...
 * @param {"gloss"|"matte"|"soft_touch"|"foil"} lamination
 * @param {object} [customerSheet] - optional customer sheet size { w, h, perProduction }
 * @returns {{ totalPrice, unitPrice, perSheet, sheetsNeeded, prodSheetsNeeded, actualQty, cuttingFee }}
 */
export function calcPaperLabelPrice(width, height, qty, shape = "rect", lamination = "gloss", customerSheet = null) {
  if (customerSheet) {
    const perCustSheet = stickersPerSheet(width, height, shape, customerSheet.w, customerSheet.h);
    const custSheetsNeeded = Math.max(1, Math.ceil(qty / perCustSheet));
    const prodSheetsNeeded = Math.max(1, Math.ceil(custSheetsNeeded / customerSheet.perProduction));
    const pricePerProdSheet = sheetPrice(prodSheetsNeeded, lamination);
    const cuttingFee = custSheetsNeeded * CUTTING_FEE_PER_SHEET;

    let totalPrice = prodSheetsNeeded * pricePerProdSheet + cuttingFee;
    totalPrice = Math.max(totalPrice, 15);

    const actualQty = custSheetsNeeded * perCustSheet;

    return {
      totalPrice: Math.round(totalPrice * 100) / 100,
      unitPrice: Math.round((totalPrice / actualQty) * 1000) / 1000,
      perSheet: perCustSheet,
      sheetsNeeded: custSheetsNeeded,
      prodSheetsNeeded,
      actualQty,
      cuttingFee: Math.round(cuttingFee * 100) / 100,
    };
  }

  // Full production sheet (12×18) — no cutting
