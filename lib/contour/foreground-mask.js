...
export function extractForegroundMask(imageData, w, h) {
  ...
  const { cleaned, componentCount, largestArea } = cleanupMask(rawMask, w, h);
...
export function createMaskOverlay(imageData, mask, w, h) {
  ...
  for (let i = 0; i < w * h; i++) {
    const isFg = mask[i] > 0;
    overlay.data[i * 4] = src[i * 4];
    overlay.data[i * 4 + 1] = src[i * 4 + 1];
    overlay.data[i * 4 + 2] = src[i * 4 + 2];
    // Foreground at full opacity, background dimmed
    overlay.data[i * 4 + 3] = isFg ? 255 : 60;
  }
  ctx.putImageData(overlay, 0, 0);

  // Draw green border around foreground region
  ctx.fillStyle = "rgba(0, 200, 0, 0.6)";
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      if (mask[y * w + x] === 0) continue;
      // Check if any 4-neighbor is background or OOB
...
function computeEdgeThreshold(gradient, w, h) {
  // Collect non-zero gradient values (skip border pixels which are 0)
  const values = [];
  for (let i = 0; i < w * h; i++) {
    if (gradient[i] > 2) values.push(gradient[i]);
  }
  if (values.length === 0) return 50;

  values.sort((a, b) => a - b);
  // 80th percentile — edges stronger than 80% of the image block flood fill
...
function gradientBasedMask(gradient, w, h) {
  ...
  const threshold = values.length > 0
    ? values[Math.floor(values.length * 0.7)]
    : 30;
...
function cleanupMask(mask, w, h) {
  ...
  const closed = morphClose(single, w, h, 2);
