...
export async function generateContour(imageUrl, opts = {}) {
  ...
  if (rawContours.length === 0) {
    // Fallback: use full-sheet rectangle instead of failing
    console.warn("[Contour] No contour found — falling back to full-sheet rectangle");
    const result = generateBasicShapeContour(origW, origH, "rectangle", {
      bleedMm: cfg.bleedMm,
      dpi: cfg.dpi,
    });
    result.processedImageUrl = processedImageUrl;
    result.bgRemoved = bgRemoved;
    result.extractionMode = extractionMode;
    result.extractionMeta = { ...extractionMeta, fallbackReason: "no_contour_found" };
    result.maskOverlayUrl = maskOverlayUrl;
    progress("done");
    return result;
  }
...
// ── Basic Shape Contour Generator ──

/**
 * Generate a contour from a basic shape preset.
 * Always succeeds — no image analysis needed.
 *
 * @param {number} imageWidth
 * @param {number} imageHeight
 * @param {"rectangle"|"square"|"circle"|"oval"} shape
 * @param {object} [opts]
 * @param {number} [opts.bleedMm=3]
 * @param {number} [opts.dpi=72]
 * @param {number} [opts.marginPercent=2] — inset from image edges (% of shorter side)
 * @returns {ContourResult}
 */
export function generateBasicShapeContour(imageWidth, imageHeight, shape = "rectangle", opts = {}) {
  const {
    bleedMm = 3,
    dpi = 72,
    marginPercent = 2,
  ...
  const margin = Math.min(imageWidth, imageHeight) * (marginPercent / 100);
  let contourPoints;

  switch (shape) {
    case "circle": {
      // Largest circle that fits inside the image (with margin)
      const cx = imageWidth / 2;
      const cy = imageHeight / 2;
      const radius = Math.min(imageWidth, imageHeight) / 2 - margin;
      contourPoints = generateEllipsePoints(cx, cy, radius, radius, 64);
      break;
    }
    case "oval": {
      // Ellipse that fills the image (with margin)
      const cx = imageWidth / 2;
      const cy = imageHeight / 2;
      const rx = imageWidth / 2 - margin;
      const ry = imageHeight / 2 - margin;
      contourPoints = generateEllipsePoints(cx, cy, rx, ry, 64);
      break;
    }
    case "square": {
      // Largest square that fits inside the image (with margin), centered
      const side = Math.min(imageWidth, imageHeight) - margin * 2;
      const ox = (imageWidth - side) / 2;
      const oy = (imageHeight - side) / 2;
      contourPoints = [
        { x: ox, y: oy },
        { x: ox + side, y: oy },
        { x: ox + side, y: oy + side },
    ...
    case "rectangle":
    default: {
      // Full-sheet rectangle (with margin)
      contourPoints = [
        { x: margin, y: margin },
  ...
  // Generate bleed offset
  const bleedPx = (bleedMm / 25.4) * dpi;
  const bleedPoints = offsetContour(contourPoints, bleedPx);

  // Generate SVG paths — use line-based path for geometric shapes
  const isRound = shape === "circle" || shape === "oval";
  const cutPath = isRound
    ? pointsToCubicBezierPath(contourPoints, true, 0.3)
    : pointsToSvgLinePath(contourPoints);
  const bleedPath = isRound
    ? pointsToCubicBezierPath(bleedPoints, true, 0.3)
    : pointsToSvgLinePath(bleedPoints);
  ...
  const quality = {
    confidence: "good",
    shapeType: shape === "rectangle" ? "rectangular" : shape,
    warnings: ["info_manual_shape"],
    suggestion: "suggestion_manual_shape",
    contourBounds: {
      ...
      width: Math.max(...contourPoints.map((p) => p.x)) - Math.min(...contourPoints.map((p) => p.x)),
      height: Math.max(...contourPoints.map((p) => p.y)) - Math.min(...contourPoints.map((p) => p.y)),
    },
    imageBounds: { width: imageWidth, height: imageHeight },
    areaCoverage: shape === "rectangle" ? 100 : shape === "oval" ? 79 : shape === "circle" ? Math.round(Math.PI / 4 * 100) : 100,
    rectangularity: shape === "rectangle" || shape === "square" ? 100 : shape === "oval" ? 79 : Math.round(Math.PI / 4 * 100),
    pointCount: contourPoints.length,
    extractionMode: "manual_shape",
    extractionMeta: { shape },
  };
 ...
 */
function generateEllipsePoints(cx, cy, rx, ry, numPoints = 64) {
  const points = [];
...
/**
 * Simple SVG line path for geometric shapes (no bezier smoothing needed).
 */
function pointsToSvgLinePath(points) {
